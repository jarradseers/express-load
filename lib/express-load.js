...
var ExpressLoad = function(entity, options, base) {
  var options = options || {};
...
ExpressLoad.prototype.then = function(location, base) {

  function add_script(script, loc, name) {
    var l = location + path.sep + name
    self.scripts.push({
      fullpath: script,
      location: base ? path.relative(true === base ? location : base, l) : l,
      name: name
    });
  }
    ...
    if (fs.statSync(entity).isDirectory()) {
      ...
      for (var file in dir) {
        ...
        if (allow && fs.statSync(script).isDirectory()) {
          this.then(path.join(location, dir[file]), true === base ? location : base);
          allow = false;
        }
        ...
        if (allow)
          add_script( script, location, getName(script) )

      }
    } else {
      add_script( entity, path.dirname(location), getName(entity) )
    }
  }
