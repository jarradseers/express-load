    ...
    if (fs.statSync(entity).isDirectory()) {
      ...
      for (var file in dir) {
        ...
        if (dir[file].charAt(0) === '.') {
          this.verbose && console.log('%s: Ignoring hidden entity: %s'
            , yellow + title + reset
            , dir[file]
          );
...
function getEntity(location) {
  if (typeof location !== 'string') {
    ...
    this.verbose && console.error('%s: File or Folder must be a string: %s'
      , red + title + reset
      , location
    ); 
  ...
  if (!fs.existsSync(entity)) {
    this.verbose && console.error('%s: No such file or directory found at %s'
      , red + title + reset
      , entity
    );
...
ExpressLoad.prototype.into = function(instance) {
  ...
  for (var s in this.scripts) {
    ...
    this.verbose && console.log('%s: Loaded *.%s'
      , green + title + reset
      , map.join('.')
    );
  }
  ...
  if (entity) {
    for (var s in this.scripts) {
      ...
      if (script.fullpath == entity || path.dirname(script.fullpath) == entity) {
        this.verbose && console.log(entity);
