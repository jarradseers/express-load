...
/**
 *  ExpressLoad automatic configuration loader.
 *
 *  @param configs {Object} autoloaded configs.
 *  @return {Object} this ExpressLoad instance.
 */

ExpressLoad.prototype.autoConfigure = function(configs) {
  var app = this.app;

  for (var config in configs) {
    app.configure(config, function() {
      for (var key in configs[config]) {
        app.set(key, configs[config][key]);
      }
    });
  }
